...
MetaFormatter for ConsoleLogger for InfrastructureModels to adjust log message format
"""
...
Helper function to create the filtered logger for InfrastructureModels
"""
function _make_filtered_logger(level)
    LoggingExtras.EarlyFilteredLogger(_LOGGER) do log
        if log._module == InfrastructureModels && log.level < level
            return false
        else
            return true
        end
